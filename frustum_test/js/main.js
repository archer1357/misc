...
function onInit() {
    ...
    mouseInput=createMouseInput(canvas);
    keyInput=createKeyInput(window);
...
function onStep(dt) {
    ...
    var move=[0,0,0];
    move[2]+=keyInput.down('w')?-1:0;
    move[2]+=keyInput.down('s')?1:0;
    move[0]+=keyInput.down('a')?-1:0;
    move[0]+=keyInput.down('d')?1:0;
    move[1]+=keyInput.down('q')?-1:0;
    move[1]+=keyInput.down('e')?1:0;
