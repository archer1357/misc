...
float calcFps() {
 ...
 void setPixel(char *pixels,int x,int y,int p, float r,float g,float b) {
    int i=(x+y*p)*3;
...
void mydofile(lua_State* L,const char *f) {
    char buffer[256];
    snprintf(buffer,256,"./%s.lua",f);
...
bool gooo() {
    HINSTANCE hinstLib = LoadLibrary("libMeinMod.dll");
    ...
    typedef int (*MYPROC)();
    MYPROC ProcAdd = (MYPROC) GetProcAddress(hinstLib, "abc");
